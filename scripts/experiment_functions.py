...
def kfold_experiments(X_complete, y_complete, cv_folds, scorer, automl_params, voter_classifiers, n_cores=1, random_state=42):
    i =0 
    ...
    for train_index, test_index in skf.split(X_complete, y_complete):
        ...
        complete_archive, best_pipeline, _ = automl(**automl_params)
        ...
        mv.fit(X_train_prep, y_train)


        # Test
        # Testing
        mv_score = SCORERS[scorer](mv, X_test_prep, y_test)
        rf_score = SCORERS[scorer](rf, X_test_prep, y_test)
        ...
        i += 1
    return kfold_archive, automl_archive
